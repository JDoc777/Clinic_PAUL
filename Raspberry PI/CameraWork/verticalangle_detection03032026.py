"""
PyQtGraph live *vertical* angular-span plot + optional Qt video preview (no Matplotlib, no cv2.imshow)

UPDATED: Ctrl+C now works reliably (Qt event loop won’t swallow SIGINT),
so the script exits and releases the camera cleanly.

- Captures frames from Picamera2 (RGB)
- YOLO runs in a worker thread (converts RGB -> BGR before inference)
- Tracks objects by *vertical* angular center and label
...
Run:
  python3 vertical_span_pyqtgraph.py
Close the window to stop, or Ctrl+C in terminal.
"""
...
HFOV_DEG = 102.0           # known/assumed horizontal FOV
# VFOV is derived from HFOV + aspect ratio (rectilinear assumption)
...
latest_tracks = []  # {"id","label","a_min","a_max","a_ctr","conf"}

det_lock = threading.Lock()
latest_dets = []  # {"x1","y1","x2","y2","label","conf"} in ORIGINAL frame coords
...
def _handle_sigint(sig, frame):
    # Make sure worker threads see the stop request ASAP
    stop_event.set()
    raise KeyboardInterrupt


# ---------------- Angle helpers (VERTICAL) ----------------
    ...
    while not stop_event.is_set():
        ...
        if boxes is not None and len(boxes) > 0:
            for box in boxes:
                ...
                a_top = pixel_y_to_angle_deg(y1, img_h, VFOV_DEG)
                a_bot = pixel_y_to_angle_deg(y2, img_h, VFOV_DEG)
                a_min = min(a_top, a_bot)
                a_max = max(a_top, a_bot)
...
class TrackPlotWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Tracked Vertical Angular Spans + Video (PyQtGraph)")
        ...
        self.plot.setLabel("left", "Vertical Angle", units="deg")
        self.plot.setYRange(-VFOV_DEG / 2.0 - 5, VFOV_DEG / 2.0 + 5)
...
def main():
    ...
    # ---- Ctrl+C fix: make Qt yield so Python can process SIGINT ----
