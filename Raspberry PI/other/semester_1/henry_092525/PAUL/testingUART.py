...
def build_command_packet(flags, motors, servos, lcd_text=""):
    ...
    payload = struct.pack(
        "<BhhhhBBBBB",  # flags, 4 x int16, 4 x uint8, lcd_len
        flags, m[0], m[1], m[2], m[3], s[0], s[1], s[2], s[3], lcd_len
    ) + txt
...
print("Payload struct size:", struct.calcsize(payload_fmt))
...
class UARTSharedData:
    ...
    def set_command_params(self, flags=None, motors=None, servos=None, lcd_text=None):
        with self.lock:
