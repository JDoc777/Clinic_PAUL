...
# Packet constants
START, END = 0x7E, 0x7F
PKT_TYPE_CMD  = 0x21   # Pi -> Arduino (command)
...
def build_command_packet(flags, motors, servos, lcd_text="", buzzer=(0, 0)):
    buzzerInt, buzzerDec = buzzer
    m = tuple(max(-255, min(255, int(x))) for x in motors)
    # pad or trim servos to exactly 5 elements
    s = list(servos) if servos else [0, 0, 0, 0, 0]
    while len(s) < 5:
        s.append(0)
    s = tuple(max(0, min(180, int(x))) for x in s[:5])
    txt = (lcd_text or "").encode("utf-8")[:64]

    payload = struct.pack("<BhhhhBBBBB", flags, m[0], m[1], m[2], m[3],
                          s[0], s[1], s[2], s[3], s[4])
    payload += struct.pack("<B", len(txt)) + txt + struct.pack("<BB", buzzerInt, buzzerDec)
...
def read_frame(ser, want_type=None, timeout=0.05):
    """
    Reads a frame from the serial port.
    """
    try:
        if ser is None:
            raise ValueError("Serial port is not initialized (ser is None).")
        ...
        print(f"[ERROR] Unexpected error in read_frame: {e}")
        return None
...
class UARTSharedData:
    ...
    def set_command_params(self, flags=None, motors=None, servos=None, lcd_text=None, buzzer=None):
        with self.lock:
            ...
            if buzzer is not None:
                self.command_params["buzzer"] = buzzer
...
def main():
    ...
    print("Starting UART write thread...")
