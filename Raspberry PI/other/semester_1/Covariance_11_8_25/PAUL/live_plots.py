...
# Import fused_pose, ekf_pose from obstacle_grid_processing
from obstacle_grid_processing import fused_pose, ekf_pose
...
def pg_live_plot_loop(grid, update_interval=20):
    ...
    win.resize(1800, 800)  # Increased height for extra plot
    ...
    # --- EKF Robot Position Plot (Trajectory) ---
    win.nextRow()
    plot_ekf_traj = win.addPlot(title="EKF Robot Position (Trajectory)")
    plot_ekf_traj.setAspectLocked(True)
    plot_ekf_traj.setXRange(-2, 2)
    plot_ekf_traj.setYRange(-2, 2)
    plot_ekf_traj.showGrid(x=True, y=True, alpha=0.3)
    ekf_traj_curve = plot_ekf_traj.plot(pen=pg.mkPen('g', width=2))
    ekf_robot_dot_traj = pg.ScatterPlotItem(size=10, brush=pg.mkBrush(0, 128, 0, 200))
    plot_ekf_traj.addItem(ekf_robot_dot_traj)
    _ekf_traj_x = []
    _ekf_traj_y = []

    # Add plot for trace(P) if needed
    # ...
    ...
    def update():
        ...
        # --- EKF trajectory update ---
        ekf_x = ekf_pose['x']
        ekf_y = ekf_pose['y']
        if not _ekf_traj_x or (abs(ekf_x - _ekf_traj_x[-1]) > 1e-4 or abs(ekf_y - _ekf_traj_y[-1]) > 1e-4):
            _ekf_traj_x.append(ekf_x)
            _ekf_traj_y.append(ekf_y)
        ekf_traj_curve.setData(_ekf_traj_x, _ekf_traj_y)
        ekf_robot_dot_traj.setData([ekf_x], [ekf_y])
