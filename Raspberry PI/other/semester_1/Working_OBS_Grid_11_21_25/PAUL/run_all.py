...
import sys
import signal
from claw import ServoController, start_claw_controller, start_claw_demo_thread, start_claw_resting
import pathfinding
...
def play_happy_birthday(shared_data, tempo=0.4, blocking=True, repeat=1):
   Payload.happy_birthday(shared_data, tempo=tempo, blocking=blocking, repeat=repeat)
...
def main():   
    ...
    happy_birthday_thread = threading.Thread(
        target=play_happy_birthday,
        args=(shared_data,),
        kwargs={'tempo': 0.4, 'blocking': True, 'repeat': 1},
        daemon=True  # Set daemon=True to ensure the thread exits when the main program exits
    )

    #happy_birthday_thread.start()
    ...
    DHTProcess = DHT_processing.create_and_run(shared_data, poll=0.1)
    ...
    Payload.set_flags_byte(shared_data, 0b00101111)  # example: set flags directly (LED1 + RESET)
    # b7:FAN2 b6:FAN1 b5:Buzzer b4:RST b3:HandSHK b2:MOTOR_ENABLE b1:led1 b0:led0

    #Payload.set_motors(shared_data, 255, 255, 225, 255)  # example: set all motors forward at 200
    #time.sleep(2)
    #Payload.set_motors(shared_data, 1, 1, 1, 1)  # example: set all motors forward at 200
    ...
    '''Payload.set_servos(shared_data, 90, 0, 90, 90, 0)  # example: set all servos to neutral (90 degrees)
    time.sleep(5)
    ...
    Payload.set_servos(shared_data, 90, 0, 90, 90, 45)  # example: set all servos to neutral (90 degrees)
    time.sleep(5)

    Payload.set_servos(shared_data, 90, 0, 90, 90, 90)  # example: set all servos to neutral (90 degrees)
    time.sleep(5)

    Payload.set_servos(shared_data, 90, 0, 90, 90, 135)  # example: set all servos to neutral (90 degrees)
    time.sleep(5)'''
