...
##########################################################################################################################################################################
    ...
    def fused_sonar_angle_processing(self, fused_pose):
        print(f"theta fused inside sonar_obstacle_local_map.py: {fused_pose['theta']}")
        self.F_sonar_angle_fused = fused_pose['theta']
        self.R_sonar_angle_fused = fused_pose['theta'] - math.radians(90)
        self.L_sonar_angle_fused = fused_pose['theta'] + math.radians(90)
        self.B_sonar_angle_fused = fused_pose['theta'] - math.radians(180)
        print(f"F_sonar_angle_fused!!!!!!: {self.F_sonar_angle_fused}")
        print(f"R_sonar_angle_fused!!!!!!: {self.R_sonar_angle_fused}")
        print(f"L_sonar_angle_fused!!!!!!: {self.L_sonar_angle_fused}")
        print(f"B_sonar_angle_fused!!!!!!: {self.B_sonar_angle_fused}")

    def fused_sonar_axis_processing(self, fused_pose):
        self.F_sonar_x_fused = fused_pose['x'] + 0.125 * math.cos(fused_pose['theta'])
        self.F_sonar_y_fused = fused_pose['y'] + 0.125 * math.sin(fused_pose['theta'])

        self.R_sonar_x_fused = fused_pose['x'] + 0.1 * math.cos(self.R_sonar_angle_fused)
        self.R_sonar_y_fused = fused_pose['y'] + 0.1 * math.sin(self.R_sonar_angle_fused)

        self.L_sonar_x_fused = fused_pose['x'] + 0.1 * math.cos(self.L_sonar_angle_fused)
        self.L_sonar_y_fused = fused_pose['y'] + 0.1 * math.sin(self.L_sonar_angle_fused)

        self.B_sonar_x_fused = fused_pose['x'] + 0.125 * math.cos(self.B_sonar_angle_fused)
        self.B_sonar_y_fused = fused_pose['y'] + 0.125 * math.sin(self.B_sonar_angle_fused)
        
        print(f"F_sonar_fused!!!!: x={self.F_sonar_x_fused:.4f} m, y={self.F_sonar_y_fused:.4f} m")
        print(f"R_sonar_fused!!!!: x={self.R_sonar_x_fused:.4f} m, y={self.R_sonar_y_fused:.4f} m")
        print(f"L_sonar_fused!!!!: x={self.L_sonar_x_fused:.4f} m, y={self.L_sonar_y_fused:.4f} m")
        print(f"B_sonar_fused!!!!: x={self.B_sonar_x_fused:.4f} m, y={self.B_sonar_y_fused:.4f} m")

    def fused_sonar_pos_processing(self):
        self.F_sensor_pos_fused = (self.F_sonar_x_fused, self.F_sonar_y_fused)
        self.R_sensor_pos_fused = (self.R_sonar_x_fused, self.R_sonar_y_fused)
        self.L_sensor_pos_fused = (self.L_sonar_x_fused, self.L_sonar_y_fused)
        self.B_sensor_pos_fused = (self.B_sonar_x_fused, self.B_sonar_y_fused)

    def F_calculate_obstacle_positions(self, angle_fused, distance_fused, sensor_pos_fused):
        angle_rad_fused = angle_fused
        # Calculate the relative (x, y) position of the obstacle
        relative_x_fused = distance_fused * math.cos(angle_rad_fused)
        relative_y_fused = distance_fused * math.sin(angle_rad_fused)

        # Calculate the absolute (x, y) position of the obstacle
        sensor_x_fused, sensor_y_fused = sensor_pos_fused
        F_obstacle_x_fused = sensor_x_fused + relative_x_fused
        F_obstacle_y_fused = sensor_y_fused + relative_y_fused

        return F_obstacle_x_fused, F_obstacle_y_fused

    def R_calculate_obstacle_positions(self, angle_fused, distance_fused, sensor_pos_fused):
        angle_rad_fused = angle_fused
        # Calculate the relative (x, y) position of the obstacle
        relative_x_fused = distance_fused * math.cos(angle_rad_fused)
        relative_y_fused = distance_fused * math.sin(angle_rad_fused)
        ...
        # Calculate the absolute (x, y) position of the obstacle
        sensor_x_fused, sensor_y_fused = sensor_pos_fused
        R_obstacle_x_fused = sensor_x_fused + relative_y_fused
        R_obstacle_y_fused = sensor_y_fused - relative_x_fused

        return R_obstacle_x_fused, R_obstacle_y_fused


    def L_calculate_obstacle_positions(self, angle_fused, distance_fused, sensor_pos_fused):
        angle_rad_fused = angle_fused
        # Calculate the relative (x, y) position of the obstacle
        relative_x_fused = distance_fused * math.cos(angle_rad_fused)
        relative_y_fused = distance_fused * math.sin(angle_rad_fused)
        ...
        # Calculate the absolute (x, y) position of the obstacle
        sensor_x_fused, sensor_y_fused = sensor_pos_fused
        obstacle_x_fused = sensor_x_fused - relative_y_fused
        obstacle_y_fused = sensor_y_fused + relative_x_fused

        return obstacle_x_fused, obstacle_y_fused
    
    def B_calculate_obstacle_positions(self, angle_fused, distance_fused, sensor_pos_fused):
        angle_rad_fused = angle_fused
        # Calculate the relative (x, y) position of the obstacle
        relative_x_fused = distance_fused * math.cos(angle_rad_fused)
        relative_y_fused = distance_fused * math.sin(angle_rad_fused)
        ...
        # Calculate the absolute (x, y) position of the obstacle
        sensor_x_fused, sensor_y_fused = sensor_pos_fused
        obstacle_x_fused = sensor_x_fused - relative_x_fused
        obstacle_y_fused = sensor_y_fused - relative_y_fused
    ...
    def fused_sonar_obstacle_axis_processing(self, fused_pose):
        self.F_obstacle_x_fused, self.F_obstacle_y_fused = self.F_calculate_obstacle_positions(fused_pose['theta'], (self.F_raw) / 100, self.F_sensor_pos_fused)
        self.R_obstacle_x_fused, self.R_obstacle_y_fused = self.R_calculate_obstacle_positions(fused_pose['theta'], (self.R_raw) / 100, self.R_sensor_pos_fused)
        self.L_obstacle_x_fused, self.L_obstacle_y_fused = self.L_calculate_obstacle_positions(fused_pose['theta'], (self.L_raw) / 100, self.L_sensor_pos_fused)
        self.B_obstacle_x_fused, self.B_obstacle_y_fused = self.B_calculate_obstacle_positions(fused_pose['theta'], (self.B_raw) / 100, self.B_sensor_pos_fused)
        ...
        print(f"B_obstacle_fused!!!!!: x={self.B_obstacle_x_fused:.4f} m, y={self.B_obstacle_y_fused:.4f} m")
