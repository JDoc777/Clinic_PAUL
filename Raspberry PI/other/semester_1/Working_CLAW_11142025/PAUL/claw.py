...
class ServoController:
    def __init__(self, step_sizes=None, transition_delay=0.01, shared_data=None, servo_directions=None):
        ...
        self.current_positions = [90, 180, 180, 90, 0]  # Initial positions
        self.target_positions = [90, 180, 180, 90, 0]   # Target positions
    ...
    def set_state(self, state):
        ...
        if state == "resting" and self.state in ["extended", "open"]:  # Allow "resting" from "open"
            self.target_positions = [90, 180, 180, 90, 0]
            self.state = "resting"
        elif state == "extended" and self.state in ["resting", "pickup"]:
            self.target_positions = [90, 45, 90, 45, self.current_positions[4] if self.state == "pickup" else 10]
            self.state = "extended"
        elif state == "open" and self.state in ["extended", "close"]:
            self.target_positions = [90, 45, 90, 45, 180]  # Ensure last servo moves to 180
            self.state = "open"
            ...
            self.target_positions = [90, 45, 90, 45, 0]  # Ensure last servo is set to 0
            self.state = "close"
            self.grabbed_object = False  # Reset the grabbed_object flag
        elif state == "pickup" and self.state == "close" and self.grabbed_object:
            self.target_positions = [90, 70, 110, 30, self.locked_close_position]
            self.state = "pickup"
        else:
