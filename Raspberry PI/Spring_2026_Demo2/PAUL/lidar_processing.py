...
class X2Parser:
    ...
    def pop_packet(self) -> Optional[Tuple[np.ndarray, np.ndarray, bool]]:
        ...
        # Dead zone filter for robot body / wheel hits
        # This is in LiDAR frame, before world transform
        xr = ranges_m * np.cos(angles_rad)
        yr = ranges_m * np.sin(angles_rad)

        dead_x = 0.25   # front/back dead zone in meters
        dead_y = 0.25   # left/right dead zone in meters

        keep = ~((np.abs(xr) < dead_x) & (np.abs(yr) < dead_y))

        angles_rad = angles_rad[keep]
        ranges_m = ranges_m[keep]
