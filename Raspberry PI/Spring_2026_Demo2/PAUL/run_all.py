...
class ToggleWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.btn_claw = QtWidgets.QPushButton("Claw Window ON")
    ...
    def toggle_claw(self):
        ...
        if claw_window is None:
            print("[Claw] No claw window exists yet.")
            return

        if claw_window.isVisible():
            claw_window.hide()
            self.btn_claw.setText("Claw Window OFF")
            print("[Claw] Window hidden.")
        else:
            claw_window.show()
            self.btn_claw.setText("Claw Window ON")
            print("[Claw] Window shown.")
...
def main():
   ...
   #t, FL, FR, BL, BR = collect_encoder_data(processor, duration=10.0)
    ...
    #Payload.set_motors(shared_data, 0, 0, 0, 0)  # Ensure motors are stopped at startup
