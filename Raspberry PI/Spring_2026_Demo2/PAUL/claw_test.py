...
# 🔥 THIS LINE FIXES YOUR WHITE BACKGROUND
        ...
        self.enable_servo_output = False
        ...
        self.servo_send_period = 0.02
        self.last_servo_tuple = None
        ...
        self.cb_enable_servos = QCheckBox("Enable Real Servo Output")
        self.cb_enable_servos.setChecked(False)
        self.cb_enable_servos.stateChanged.connect(
            lambda: setattr(self, "enable_servo_output", self.cb_enable_servos.isChecked())
        )
        manual_layout.addWidget(self.cb_enable_servos)
    ...
    def set_servo_angles_direct(self, yaw=None, shoulder=None, elbow=None, wrist=None, claw=None):
        ...
        if self.shared_data is not None and self.enable_servo_output:
            Payload.set_servos(
                self.shared_data,
                current[0],
                current[1],
    ...
    def _tick(self):
        ...
        if self.enable_servo_output:
            if not self.grip_locked and not self.manual_mode:
                if servos != self.last_servo_tuple and now - self.last_servo_send_time >= self.servo_send_period:
                    Payload.set_servos(
                        self.shared_data,
                        servos[0],
                        servos[1],
