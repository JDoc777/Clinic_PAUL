...
# ============================================================
# JOINT LIMITS — each servo is 180° total, centred on HOME
#
#  Joint      Home      Range            Notes
#  --------   -------   --------------   ----------------------
#  yaw          0°      -90° … +90°      centred straight-forward
#  shoulder    +45°     -45° … +135°     home = arm angled up-forward
#  elbow       -90°    -180° …   0°      home = forearm horizontal
#  wrist         0°     -90° … +90°      centred straight-through
# ============================================================
JOINT_LIMITS = {
...
# Home pose — all servos at their mechanical mid-point
HOME_ANGLES = [
    0.0,               # yaw      =   0°
    math.radians( 45), # shoulder = +45°
    math.radians(-90), # elbow    = -90°
    0.0,               # wrist    =   0°  (straight through)
]
...
class IKWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PAUL Arm IK  —  180° Servo Limits")
        ...
        # Sync chassis mesh to origin and draw home pose before first tick
        self._sync_chassis()
        self._refresh_draw()
    ...
    def move_to_target(self):
        ...
        # 1. Direct IK + clear straight path
        ...
        if sol is not None:
            if not path_collides(self.current_angles,sol,cp,self.obstacles):
                ...
                return
            # Path sweeps obstacle — try via-point
