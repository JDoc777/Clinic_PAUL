...
//initialize 2 fans as outputs a7 and a5 inside of setup and pins, trigger on or off based off true or false in apply command
...
void lcd_boot_animation() {
    static bool lcdBootDone = false;
    if (lcdBootDone) return;   // DO NOT RUN TWICE
    lcdBootDone = true;

    const char* title = "PAUL STARTING...";
    const char* frames[] = {
        "[              ]",
        "[=             ]",
        "[==            ]",
    ...
    for (int i = 0; i < numFrames; i++) {
        ...
        lcd_show_text(buf, strlen(buf), true);
        delay(150);
    }

    lcd_show_text("   PAUL READY!\n[==============]", 32, true);
